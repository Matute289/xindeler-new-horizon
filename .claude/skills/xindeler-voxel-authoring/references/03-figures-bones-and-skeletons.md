...
The 39 wrapped wildcards in `common/src/comp/body/mod.rs` are the other merge
surface: upstream edits `_ =>` lines in that file a handful of times a year, so
expect the occasional one-line conflict, resolved by keeping our wrapper and
their value.
