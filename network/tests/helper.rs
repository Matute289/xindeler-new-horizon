...
use xindeler_network::{ConnectAddr, ListenAddr, Network, Participant, Pid, Promises, Stream};
...
pub fn setup(tracing: bool, sleep: u64) -> (u64, u64) {
    ...
    let _subscriber = if tracing {
        let filter = EnvFilter::from_default_env()
            ...
            .add_directive("xindeler_network::tests=trace".parse().unwrap())
            .add_directive("xindeler_network::controller=trace".parse().unwrap())
            .add_directive("xindeler_network::channel=trace".parse().unwrap())
            .add_directive("xindeler_network::message=trace".parse().unwrap())
            .add_directive("xindeler_network::metrics=trace".parse().unwrap())
            .add_directive("xindeler_network::types=trace".parse().unwrap());
