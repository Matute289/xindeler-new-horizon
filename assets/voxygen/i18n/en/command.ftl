...
command-transcribe_spell-desc = (Admin) Transcribe a page from the target's equipped Tome into their spellbook, going through the same class/possession/mastery/cost gates a real transcription would
...
command-pact-desc = (Admin) Manage a Warlock's pact: 'bind <patron> [target]' binds them to a patron, 'sever [target]' breaks the pact (disabling their magic), 'boon <tome|chain|blade|talisman> [target]' sets their pact boon, 'blade <summon|dismiss> [target]' toggles their conjured blade (Blade boon only), 'status [target]' reports it
