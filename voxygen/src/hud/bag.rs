...
/// Builds the per-slot tint closure that grays out items the **viewer** cannot
/// equip, i.e. whose level/class/skill requirements the viewer does not meet.
///
/// The gate is always evaluated against the local player, even when the grid is
/// showing someone else's inventory (a trade counterparty's pane): the question
/// the gray-out answers is "could *I* use this?".
///
/// Returned as a closure so the three call sites (`InventoryMenu`'s grid,
/// `GearMenu`'s gear-filtered grid and `InventoryScroller`'s trade pane) share
/// one definition; it is handed to `SlotGrid::slot_tint`, which applies it
/// after its own loadout-hover/salvage/overflow highlights so the gray wins on
/// conflict.
fn equip_requirement_tint(client: &Client) -> impl Fn(Slot, Option<&Item>) -> Option<Color> + '_ {
    ...
    move |_pos: Slot, item: Option<&Item>| -> Option<Color> {
        let skill_set = skill_sets.get(player_entity)?;
        let body = bodies.get(player_entity)?;
        let player_class = character_classes.get(player_entity);
        let item = item?;
...
impl<'a> InventoryScroller<'a> {
    ...
    fn scrollbar_and_slots(
        ...
        // unfocused keeps a stray menu highlight off the counterparty's first slot.
        .is_focused(false)
...
impl Widget for BagWindow<'_> {
    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        // The bag is force-opened alongside Crafting/Trade (both call
        // `set_bag_state` so their own slot grids can show the player's
        // inventory), and Crafting/Trade consume `MenuInput`s of their own for
        // their own navigation. With the bag's grids and paperdoll also
        // navigable at the same time, a single dpad press would move both this
        // window's highlight AND Crafting's/Trade's, and `Apply` could open this
        // window's Use/Drop/Cancel context menu on whatever slot happened to be
        // highlighted here — a real risk of an unintended item drop/use while
        // trying to navigate Crafting/Trade. Crafting and Trade each handle their
        // own `MenuInput::Back` independently of this window's `Close` events, so
        // suppressing navigation here costs nothing: Back still closes both
        // windows correctly.
        let bag_nav_allowed = !(self.show.crafting || self.show.trade);
...
impl<'a> InventoryMenu<'a> {
    ...
    /// Whether this menu's grid may consume menu (dpad/keyboard) navigation
    /// this frame. `false` while another window that owns `MenuInput`s of its
    /// own is open alongside the bag.
    pub fn nav_allowed(mut self, nav_allowed: bool) -> Self {
        self.nav_allowed = nav_allowed;
        self
    }
...
impl<'a> GearMenu<'a> {
    ...
    /// Whether the paperdoll and the gear grid may consume menu
    /// (dpad/keyboard) navigation this frame. `false` while another window that
    /// owns `MenuInput`s of its own is open alongside the bag.
    pub fn nav_allowed(mut self, nav_allowed: bool) -> Self {
        self.nav_allowed = nav_allowed;
        self
    }
...
impl Widget for GearMenu<'_> {
    ...
    fn update(self, args: widget::UpdateArgs<Self>) -> Self::Event {
        ...
        // Back: closes the bag
        if self.nav_allowed && state.is_focused {
            for event in self.menu_events {
                match *event {
        ...
        // Stats
        let block_gear_tab = self.show.bag_menu_split && !self.show.trade;
        // The viewpoint entity's cached gear aggregates. Attunement gating is
        // already folded into `protection`, so the Protection display reflects
        // it rather than the raw armour value, without this widget walking the
        // loadout every frame.
        ...
        let derived = derived_stats.get(self.tab_package.info.viewpoint_entity);
        ...
        for i in STATS.iter().copied().enumerate() {
            ...
            let stun_res_txt = format!(
                "{}",
                (100.0 * Poise::compute_poise_damage_reduction(derived, None, Some(self.stats)))
                    as i32
            );
