...
/// Screen-space ambient occlusion tap-count tier. Higher tiers spend more
/// samples per pixel for a quieter result at a higher cost; read back by
/// `ssao-frag.glsl` as `SSAO_QUALITY_LOW` / `SSAO_QUALITY_MEDIUM` /
/// `SSAO_QUALITY_HIGH`.
...
pub struct SsaoConfig {
    ...
    /// 0.0..1.0, how strongly the occlusion generated by the SSAO pass
    /// darkens the volumetric (clouds) pass's output. This multiplies the
    /// forward shaders' already-combined emitted+reflected radiance rather
    /// than an isolated ambient term, because no separate ambient channel
    /// exists in this engine's forward shaders -- kept as a conservative
    /// default with a floor rather than pushed to 1.0 so that approximation
    /// never fully blacks out direct light.
    pub strength: f32,
}
...
impl Default for SsaoMode {
    fn default() -> Self { Self::On(SsaoConfig::default()) }
}
...
impl PipelineModes {
    pub fn remove_unsupported(&mut self) {
        ...
        // The screen-space bind layouts SSAO shares with bloom/postprocess
        // (`clouds.rs`, `postprocess.rs`) hard-code `multisampled: false`
        // while the render targets are created with the active sample
        // count, a pre-existing mismatch this row doesn't attempt to fix
        // (MSAA is already disabled in the Video-settings UI for the same
        // reason). Force SSAO off rather than let it hit that mismatch.
        if self.ssao.is_on() && self.aa.samples() > 1 {
            warn!("SSAO is not currently supported with MSAA enabled and will be disabled.");
            self.ssao = SsaoMode::Off;
        }
    }
}
